...
const LOADING_STATUS_MESSAGES = [
  "Initializing GPU shader simulation...",
  "Standardizing hardware instruction sets...",
  "Querying reference database structures...",
  "Evaluating CPU/GPU thread parity...",
  "Running 3D rendering pipeline simulations...",
  "Formulating bottlenecks & upgrade recommendations...",
  "Structuring diagnostic final report..."
];
...
export default function Analyzer({ onAnalysisSuccess, remainingAnalyses, setRemainingAnalyses }: AnalyzerProps) {
  ...
  // Cycle loading messages for a premium engaging diagnostic experience
  useEffect(() => {
          ...
          {loading ? (
            ...
            >
              <div className="relative flex h-20 w-20 items-center justify-center mb-8">
                <motion.div 
                  className="absolute inset-0 rounded-2xl border-2 border-dashed border-cyan-500/20"
                  animate={{ rotate: -360 }}
                  transition={{ repeat: Infinity, duration: 25, ease: "linear" }}
                />
                <motion.div 
                  className="absolute inset-1.5 rounded-xl border border-cyan-400/30 bg-cyan-950/40"
                  animate={{ scale: [1, 1.05, 1] }}
                  transition={{ repeat: Infinity, duration: 2, ease: "easeInOut" }}
                />
                <RefreshCw className="h-8 w-8 text-cyan-400 animate-spin z-10" />
              </div>
              
              <h3 className="text-xl font-bold text-white tracking-tight font-display">
                Analyzing Hardware Thread Metrics
              </h3>
              
              <div className="mt-4 px-4 py-2 rounded-lg bg-zinc-900/60 border border-zinc-800 font-mono text-xs text-cyan-400 tracking-wider uppercase min-w-[320px] text-center shadow-inner">
                {LOADING_STATUS_MESSAGES[loadingMsgIdx]}
              </div>

              <p className="mt-8 text-xs text-zinc-500 max-w-sm leading-relaxed">
                Evaluating core structures, thermal tolerances, and floating-point shader processing models.
              </p>
            </motion.div>
          ) : (
