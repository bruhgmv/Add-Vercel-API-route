...
export default function Footer({ setView }: FooterProps) {
  const handleLinkClick = (viewId: ViewState) => {
    setView(viewId);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const footerLinks = [
    { label: "About Us", viewId: "about" },
    { label: "Privacy Policy", viewId: "privacy" },
    { label: "Terms of Service", viewId: "terms" },
    { label: "Cookie Policy", viewId: "cookies" },
    { label: "Contact Info", viewId: "contact" }
  ] as const;
    ...
    <footer className="border-t border-zinc-900 bg-zinc-950 py-12 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center border-b border-zinc-900 pb-8 mb-8">
          ...
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-2">
            {footerLinks.map((link) => (
              <button
                key={link.viewId}
                onClick={() => handleLinkClick(link.viewId)}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-[11px] text-zinc-500 leading-normal text-center sm:text-left max-w-lg">
